...
const ListByBrandService = async (req) => {
    try {
        // param name must match the route (brandID, not BrandID)
        ...
        console.error("Error fetching products by brand:", e);
        return { status: "fail", data: e.toString() };
    }
};
...
const ListByRemarkService = async (req) => {
    try {
        ...
        console.log("Incoming remark:", Remark);

        let MatchStage = {
            $match: {
                remark: Remark   // first test with exact match
            }
        };
        ...
        console.error("Error fetching products by remark:", e);
        return { status: "fail", data: e.toString() };
    }
};
